import { BAIRROS, INTENTS, BRAND } from "@/lib/seoData";
    ...
    return (
        <>
            ...
            <section className="container">
                <div className="grid">
                    {BAIRROS.map(bairro => (
                        <div key={bairro} className="card" style={{ padding: '20px' }}>
                            <h3 style={{ fontSize: '1.2rem', textTransform: 'capitalize' }}>{bairro.replace(/-/g, ' ')}</h3>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginTop: '15px' }}>
                                {INTENTS.map(intent => (
                                    <Link
                                        key={intent.slug}
                                        href={`/curitiba/${bairro}/${intent.slug}/`}
